...
全文通常只将 1--3 个承担关键结果、最终决策或主要方法贡献的算法标为核心。每个核心算法必须提供伪代码；辅助算法、常规数据清洗、单个统计量和可由一条闭式公式完整表达的步骤不机械增加伪代码。
...
`pseudocode_anchor` 必须对应正文 `algorithm` 环境内部的 `\label{...}`。校验器同时检查该环境是否包含 `\caption`、`\KwIn` 和 `\KwOut`。辅助算法将两个字段留空。
